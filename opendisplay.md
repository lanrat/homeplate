...
- **Read-only configuration over OpenDisplay.** The device answers `READ_CONFIG` (`0x0040`) with the minimum 4-packet TLV blob py-opendisplay's parser requires (SYSTEM, MANUFACTURER, POWER, DISPLAY), populated with panel dimensions, color scheme, partial-update flag, and the configured deep-sleep interval. The optional packets (LED, SENSOR, TOUCH, WIFI, SECURITY, etc.) are omitted. `READ_FW_VERSION` (`0x0043`) still returns a bare ACK. `WRITE_CONFIG` / `WRITE_CONFIG_CHUNK` (`0x0041`/`0x0042`) are not implemented — use HomePlate's existing setup portal / MQTT entities for configuration instead.
- **Color scheme per board, with caveats:**
  - **B&W grayscale boards** (5, 5v2, 6, 6v2, 6 Plus, 6 Plus v2, 6 Flick, 10, 10v2) report `GRAYSCALE_16` (4 bits per pixel, 16 levels). The server dithers to 16 levels; HomePlate downsamples each received 4-bit value to the panel's native 3-bit grayscale (`>> 1`) for the render.
  - **Inkplate 6 COLOR** (7-color ACeP) reports `BWGBRY` (the OpenDisplay 6-color scheme). The server dithers to the 6-color palette (black/white/yellow/red/blue/green) and HomePlate maps each received nibble to the matching `INKPLATE_*` color constant. **The panel's 7th color (orange) is unused** — the OpenDisplay basic/Flex color schemes don't define it.
